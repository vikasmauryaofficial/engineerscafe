...
def food_pagecat(request,ak):
    dic={}
    ct=Categorie.objects.get(id=ak)
    obj=food.objects.filter(categ=ct)
    dic["fds"]= obj

    return render(request,"food_page.html",dic)
def food_page2(request):
    return render(request,"food-page2.html")
def gallery(request):
...
def single_food(request):
    return render(request,"single_food.html") 
